...
class ngc_z1_marg(Likelihood_prior):
    ...
    def __init__(self,path,data,command_line):
        ...
        # First load in data
        if hasattr(self,'cut_k_size'):
            self.ksize = self.cut_k_size
            print("Using %d bins"%self.ksize)
        ...
        # Load in covariance being careful about the number of bins
        all_cov = np.loadtxt(os.path.join(self.data_directory,self.covmat_file))
        n_cov = len(all_cov)//2
        self.cov[:self.ksize,:self.ksize] = all_cov[:self.ksize,:self.ksize]
        self.cov[:self.ksize,self.ksize:] = all_cov[:self.ksize,n_cov:n_cov+self.ksize]
        self.cov[self.ksize:,:self.ksize] = all_cov[n_cov:n_cov+self.ksize,:self.ksize]
        self.cov[self.ksize:,self.ksize:] = all_cov[n_cov:n_cov+self.ksize,n_cov:n_cov+self.ksize]
        if hasattr(self,'rescale_cov'):
            print("Rescaling covariance by a factor of %.2f"%self.rescale_cov)
            self.cov/=self.rescale_cov
    ...
    def loglkl(self, cosmo, data):
        ...
        b2mean,bG2mean,css0mean,css2mean,Pshotmean,b4mean = self.prior_means
        b2sig,bG2sig,css0sig,css2sig,Pshotsig,b4sig = self.prior_sigs
        ...
        # Add priors
        chi2 += (b2 - b2mean)**2./b2sig**2. + (bG2 - bG2mean)**2/bG2sig**2. #+ (css0)**2/30**2 + css2**2/30**2 + (b4-500.)**2/500**2 + (Pshot - 5e3)**2./(5e3)**2.
